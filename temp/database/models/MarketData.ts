...
MarketDataSchema.statics.findArbitrageOpportunities = async function (
  ...
  const pipeline: PipelineStage[] = [
    // Group by token pair and get latest price for each exchange
    {
      $group: {
        _id: {
          tokenA: '$tokenA',
    ...
    // Group again to compare prices between exchanges
    {
      $group: {
        _id: {
          tokenA: '$_id.tokenA',
    ...
    // Filter for pairs with price differences
    {
      $match: {
        'exchanges.1': { $exists: true }, // Ensure we have at least 2 exchanges
      },
    } as PipelineStage,
    // Calculate spread
    {
