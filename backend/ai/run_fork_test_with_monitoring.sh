...
# Configuration
DURATION=${1:-24}  # Default to 24 hours if not specified
TRADES_PER_HOUR=${2:-10}  # Default to 10 trades per hour
METRICS_DIR="backend/ai/metrics/fork_test"
RESULTS_DIR="backend/ai/results/fork_test"
LOG_FILE="fork_test_with_monitoring.log"

# Print banner
echo "=========================================================="
echo "  ArbitrageX Forked Mainnet Test with Monitoring"
echo "=========================================================="
echo "Duration: $DURATION hours"
echo "Trades per hour: $TRADES_PER_HOUR"
...
# Start the test
echo "Starting forked mainnet test..."
echo "This will run for $DURATION hours with $TRADES_PER_HOUR trades per hour."
echo ""
echo "Press Ctrl+C to stop the test at any time."
echo ""

# Run the test
python backend/ai/run_fork_test_with_monitoring.py \
    --duration "$DURATION" \
    --trades "$TRADES_PER_HOUR" \
