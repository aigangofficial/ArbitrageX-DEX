...
class PriceAggregator extends events_1.EventEmitter {
    ...
    checkArbitrageOpportunities(symbol) {
        ...
        for (const [cexName, cexPrice] of Object.entries(priceData.cexPrices)) {
            for (const [dexName, dexPrice] of Object.entries(priceData.dexPrices)) {
                const spread = this.calculateSpread(bignumber_1.BigNumber.from(cexPrice), bignumber_1.BigNumber.from(dexPrice));
                if (spread.gte(this.MIN_PROFIT_THRESHOLD)) {
                    opportunities.push({
                        ...
                        estimatedProfit: this.estimateProfit(bignumber_1.BigNumber.from(cexPrice), bignumber_1.BigNumber.from(dexPrice)).toString(),
                        timestamp: Date.now()
                    });
                }
            }
    ...
    estimateProfit(price1, price2) {
        ...
        const amount = bignumber_1.BigNumber.from('1000000000000000000');
