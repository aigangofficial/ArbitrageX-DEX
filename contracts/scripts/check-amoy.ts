...
async function main() {
  ...
  try {
    ...
    console.log(
      `Base Fee Per Gas: ${block.baseFeePerGas ? ethers.formatUnits(block.baseFeePerGas.toString(), 'gwei') : '0'} Gwei`
    );
    ...
    console.log('\nGas Price:', {
      maxFeePerGas: gasPrice.maxFeePerGas
        ? ethers.formatUnits(gasPrice.maxFeePerGas.toString(), 'gwei') + ' gwei'
        : 'Not available',
      maxPriorityFeePerGas: gasPrice.maxPriorityFeePerGas
        ? ethers.formatUnits(gasPrice.maxPriorityFeePerGas.toString(), 'gwei') + ' gwei'
        : 'Not available',
      gasPrice: gasPrice.gasPrice
        ? ethers.formatUnits(gasPrice.gasPrice.toString(), 'gwei') + ' gwei'
        : 'Not available',
    });
