...
async function main() {
  console.log(
    colors.blue + `\n🔍 Checking balances on ${network.name.toUpperCase()}...\n` + colors.reset
  );
  ...
  console.log(`Network: ${colors.yellow}${network.name.toUpperCase()}${colors.reset}`);
  ...
  console.log(
    `Base Fee: ${colors.yellow}${ethers.formatUnits(feeData.gasPrice || 0n, 'gwei')}${
      colors.reset
    } gwei`
  );
  console.log(
    `Max Priority Fee: ${colors.yellow}${ethers.formatUnits(
      feeData.maxPriorityFeePerGas || 0n,
      'gwei'
    )}${colors.reset} gwei`
  );
  console.log(
    `Max Fee: ${colors.yellow}${ethers.formatUnits(feeData.maxFeePerGas || 0n, 'gwei')}${
      colors.reset
    } gwei\n`
  );

  // Check if balance is sufficient for deployment
  const minBalance = ethers.parseEther(network.name === 'amoy' ? '0.5' : '0.1');
  if (balance < minBalance) {
    console.log(
      colors.red +
        `⚠️  Insufficient balance for deployment. Please fund this address with at least ${
          network.name === 'amoy' ? '0.5 MATIC' : '0.1 ETH'
        }\n` +
        colors.reset
    );
    ...
    console.log(
      colors.green +
        `✅ Balance is sufficient for deployment (${ethers.formatEther(balance)} ${currency})\n` +
        colors.reset
    );
  }
}
