...
describe('MockPool', () => {
    ...
    beforeEach(async function () {
        ...
        mockPool = await MockPool.deploy(await mockTokenA.getAddress(), await mockTokenB.getAddress(), owner.address);
    ...
    describe('Flash Loan Functionality', function () {
        it('should execute flash loan with proper repayment', async function () {
            ...
            const modes = [0];
            await (0, chai_1.expect)(mockPool.flashLoan(await receiver.getAddress(), assets, amounts, modes, owner.address, '0x', 0)).to.not.be.reverted;
        ...
        it('should revert if loan is not repaid', async function () {
            ...
            const modes = [0];
            await (0, chai_1.expect)(mockPool.flashLoan(await receiver.getAddress(), assets, amounts, modes, owner.address, '0x', 0)).to.be.revertedWith('Flash loan not repaid');
        });
        it('should charge correct flash loan fee', async function () {
            ...
            const modes = [0];
            // Execute flash loan
            await mockPool.flashLoan(await receiver.getAddress(), assets, amounts, modes, owner.address, '0x', 0);
    ...
    describe('Edge Cases', () => {
        it('should handle zero amount flash loans', async () => {
            ...
            const amounts = [0];
            const modes = [0];
            await (0, chai_1.expect)(mockPool.flashLoan(await receiver.getAddress(), assets, amounts, modes, owner.address, '0x', 0)).to.be.revertedWith('Invalid amount');
        });
        it('should handle multiple flash loans in the same transaction', async () => {
            ...
            const modes = [0, 0];
            await (0, chai_1.expect)(mockPool.flashLoan(await receiver.getAddress(), assets, amounts, modes, owner.address, '0x', 0)).to.be.revertedWith('Only single asset flash loans supported in mock');
        });
        it('should validate input parameters', async () => {
            ...
            const amounts = [FLASH_LOAN_AMOUNT, FLASH_LOAN_AMOUNT];
            const modes = [0];
            await (0, chai_1.expect)(mockPool.flashLoan(await receiver.getAddress(), assets, amounts, modes, owner.address, '0x', 0)).to.be.revertedWith('Array lengths must match');
        });
    });
});
